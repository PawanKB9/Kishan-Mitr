...
const UpdateDetails = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim() && !shopName.trim() && !location) return alert("1 field is required");
    ...
    try {
      await updateUser({name , shopName ,location}).unwrap();
  ...
  return (
    <div className="w-full max-w-lg mx-auto mt-16 md:mt-20 px-6 py-8 bg-white rounded-lg shadow-md border border-gray-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Shop Name */}
        <div>
          <label className="block text-sm sm:text-base font-medium text-gray-700 mb-1">Shop Name</label>
          <input
            type="text"
            className="w-full sm:text-lg border border-gray-300 rounded px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-green-500"
